...
UpgradeMenu::UpgradeMenu() {
    ...
    // Boton de dano.
    damageButton.setSize(sf::Vector2f(420.f, 44.f));
    damageButton.setFillColor(sf::Color(70, 45, 85));

    // Boton de rango.
    rangeButton.setSize(sf::Vector2f(420.f, 44.f));
    rangeButton.setFillColor(sf::Color(55, 70, 90));

    // Boton de capacidad.
    capacityButton.setSize(sf::Vector2f(420.f, 44.f));
    capacityButton.setFillColor(sf::Color(75, 65, 45));

    // Boton para continuar al siguiente piso.
    continueButton.setSize(sf::Vector2f(360.f, 42.f));
...
void UpgradeMenu::layout(sf::RenderWindow& window, Progression& progression) {
    ...
    // Textos dinamicos segun oro, score y niveles actuales.
    ...
    // Centramos los textos dentro de sus zonas.
...
UpgradeAction UpgradeMenu::handleEvent(sf::Event& event, sf::RenderWindow& window) {
    ...
    if (event.type == sf::Event::MouseButtonPressed &&
        ...
        if (capacityButton.getGlobalBounds().contains(mouse)) {
            return UpgradeBuyCapacity;
        }
...
void UpgradeMenu::draw(sf::RenderWindow& window, Progression& progression) {
    ...
    panel.setSize(sf::Vector2f(560.f, 380.f));
    panel.setOrigin(280.f, 190.f);
